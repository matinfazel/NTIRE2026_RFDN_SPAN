...
class RFDB_PFA(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.25):
        super(RFDB_PFA, self).__init__()
    ...
    def forward(self, input):
        ...
        # SPAN-style parameter-free attention
        v_i = torch.sigmoid(h_i) - 0.5
        u_i = h_i + input
...
class RFDN_SPAN(nn.Module):
    def __init__(self, in_nc=3, nf=50, num_modules=4, out_nc=3, upscale=4):
        super(RFDN_SPAN, self).__init__()

        self.fea_conv = conv_layer(in_nc, nf, kernel_size=3)

        self.B1 = RFDB_PFA(in_channels=nf)
        self.B2 = RFDB_PFA(in_channels=nf)
        self.B3 = RFDB_PFA(in_channels=nf)
        self.B4 = RFDB_PFA(in_channels=nf)
